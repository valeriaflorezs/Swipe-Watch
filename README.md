...
| Recomendación | k-Nearest Neighbors (k-NN) | Clasifica películas candidatas basándose en la similaridad con las películas que el usuario ha gustado. |
| Métrica | Distancia Euclídea | Mide la similitud entre los vectores de género de las películas. |
| Modelado de Datos | Grafo Bipartito Ponderado | Representa la relación Usuario-Género mediante una matriz de adyacencia. |
| API de Datos | The Movie Database (TMDB) | Obtención de datos de películas (títulos, pósters, géneros). |
| Visualización | NetworkX, Matplotlib, Pandas | Creación del grafo de afinidad y manejo de datos. |
